...
/* ── 插件窗毛玻璃（B站@角落里暗伤 建议）──
	其他插件的小窗（body 级 fixed 浮层，由 frostWindow 打标 .qx-frost）
	背后自动加一层背景虚化，半透明窗口不再透出壁纸糊住文字。
	虚化必须放 ::before 伪元素：backdrop-filter 直接挂在窗口元素上会把
	其 fixed 后代（内嵌弹窗/取色器）的包含块困进窗口——v0.1.2 弹窗塌缩
	同款陷阱。伪元素无后代，零副作用；z-index:-1 落在窗口自身层叠上下文
	内（fixed+z 必然成上下文），位于窗口底色之上、内容之下。 */
body[data-dsh-qingxiao] .qx-frost::before {
	content: "";
	position: absolute;
	inset: 0;
	z-index: -1;
...
`;
		...
		function apply(ctx) {
			...
			// ── 插件窗毛玻璃：给 body 级 fixed 浮层打标 .qx-frost（CSS 见 THEME_CSS 末尾）──
			// 只观察 body 直接子节点（插件窗一律 append 到 body），不用 subtree，
			// 流式渲染时零开销。z-index>=10 守卫把 DSH 应用主框排除在外（其浮层在
			// frame 的 overlayLayer 里，本已由皮肤令牌玻璃化，不经此处）。
			var QX_FROST_CLASS = "qx-frost";
			function frostWindow(el) {
				if (!el || el.nodeType !== 1) return;
				if (el.hasAttribute("data-skin-chrome")) return;
				if (el.classList.contains(QX_FROST_CLASS)) return;
				var st = getComputedStyle(el);
				if (st.position !== "fixed" || st.display === "none" || st.visibility === "hidden") return;
				if (!(parseInt(st.zIndex, 10) >= 10)) return;
				...
				var unfrostIfApp = function() {
					if (el.isConnected && el.querySelector("[data-dsh-sidebar-root], [data-conversation-scroll]")) {
						el.classList.remove(QX_FROST_CLASS);
					}
				};
			...
			var frostObserver = new MutationObserver(function(muts) {
				for (var i = 0; i < muts.length; i++) {
					...
					for (var j = 0; j < added.length; j++) frostWindow(added[j]);
				}
			});
			frostObserver.observe(document.body, { childList: true });
			// 皮肤挂载前就已存在于 body 的插件浮层（启动即挂载的常驻窗）补扫一遍。
			// ponytail: display:none 预挂载、之后纯靠改 style 显示的窗会漏（无 DOM 新增可观察）；
			// 实测主流插件窗都是点击时即时 append，若将来遇到漏网的再补 visibility 级观察。
			Array.prototype.forEach.call(document.body.children, frostWindow);
