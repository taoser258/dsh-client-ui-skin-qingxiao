...
- **毛玻璃虚化**：总开关 + 「插件独立窗 / 原生面板」分项子开关 + 0–30px 独立强度滑块。用量统计等独立插件窗、左右两侧面板与 overlayLayer 内的插件窗都会垫一层背景虚化，隐约透出壁纸而不糊字；关掉的分项整条 CSS 不匹配，`backdrop-filter` 彻底移除、不留 GPU 合成层开销。
...
| 毛玻璃虚化 | 总开关 · 插件独立窗 · 原生面板（侧栏 / 详情栏 / 文件面板 / 底部面板） · 虚化强度 0–30px（0＝关闭） |
